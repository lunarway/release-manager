...
func NewRollback(client *client.ReleaseManagerServerAPI, clientAuth *runtime.ClientAuthInfoWriter, service *string) *cobra.Command {
...
has no effect.`,
  ...
  hamctl rollback --service product --env dev`,
		...
		RunE: func(c *cobra.Command, args []string) error {
			var currentRelease *models.DescribeReleaseResponseReleasesItems0
			var rollbackTo *models.DescribeReleaseResponseReleasesItems0

			if artifactID == "" {

				releasesResponse, err := actions.ReleasesFromEnvironment(client, clientAuth, *service, environment, 10)
				if err != nil {
					return err
				}
			...
			resp, err := actions.ReleaseArtifactID(client, clientAuth, *service, environment, rollbackTo.Artifact.ID, intent.NewRollback(currentRelease.Artifact.ID))
			if err != nil {
