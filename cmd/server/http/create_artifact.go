...
func createArtifact(payload *payload, artifactWriteStorage ArtifactWriteStorage) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		logger.Infof("http: artifact: creating artiact for '%s' hash '%x'", req.Artifact.ID, req.MD5Hash)
		uploadURL, err := artifactWriteStorage.CreateArtifact(req.Artifact, req.MD5Hash)
		if err != nil {
...
type ArtifactWriteStorage interface {
	...
	CreateArtifact(artifactSpec artifact.Spec, md5hash string) (string, error)
}
